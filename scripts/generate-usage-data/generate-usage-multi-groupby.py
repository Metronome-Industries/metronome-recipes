...
if events_per_hour > 1000 or events_per_hour < 1:
    raise ValueError(
        "events_per_hour needs to be greater than 1 and less than 1000")

days_of_data = args.days_of_data
if days_of_data > 33 or days_of_data < 1:
    raise ValueError(
        "days_of_data needs to be greater than 1 and less than 34")
...
while t < datetime.now(timezone.utc):
    ...
    for index in range(events_per_hour):
        append_event('KafkaNetworkRead', {
            ...
            "resource.infra": random.choice(["basic", "custom"]),
            ...
            "resource.account_id": random.choice(["account-1", "account-2"]),
            "resource.region": random.choice(["eu-west-1", "us-west-2"]),
        })
    t += timedelta(hours=1)
