...
while t < datetime.now(timezone.utc):
    ...
    developer_seats = random.randint(0, 25)
    developer_seats_in_use = random.randint(0, developer_seats)
    read_only_seats= random.randint(0, 140)
    read_only_seats_in_use = random.randint(0, read_only_seats)
    
    for index in range(events_per_hour):
        append_event('account_heartbeat', {
                "developer_seats": developer_seats,
                "developer_seats_in_use": developer_seats_in_use,
                "read_only_seats": read_only_seats,
                "read_only_seats_in_use": read_only_seats_in_use,
        })
    t += timedelta(hours=1)
